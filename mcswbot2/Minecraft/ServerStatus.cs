...
namespace mcswbot2.Minecraft
{
    ...
    {
        private static readonly List<ServerStatusWatcher> _watchers = new();
        ...
        {
            Parallel.ForEach(_watchers, w => { w.Execute(timeOutMs); });
        }
        ...
        private void RegisterWatcher(string address, int port, bool reuse = true)
        {
            ...
            var res = _watchers.FindAll(wtch =>
        ...
        private EventBase[] Update(ServerInfoExtended current)
        {
            ...
            {
                Debug.WriteLine("Server '" + Watcher.Address + ":" + Watcher.Port + "' status change: " +
                                current.HadSuccess);
