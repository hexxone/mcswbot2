...
namespace McswBot2.Minecraft
{
    internal class ServerInfo
    {
        ...
        private ServerInfoExtended Get(CancellationToken ct, DateTime dt, TcpClient client, NetworkStream stream)
        {
            ...
            var pingRespData = Read(ref offset, readBuffer, 8);
            var pingRndResponse = BitConverter.ToInt64(pingRespData);

            // check for packet
            if (packetNr == 1 && sentRnd == pingRndResponse)
            {
                sw.Stop();
            }
            else
            {
                Logger.WriteLine("Ping Data mismatch: " + sentRnd + " => " + pingRndResponse);
            }
        ...
        private static string GetDescription(dynamic desc)
        {
            ...
            try
            {
                ...
                if (desc.extra != null)
                {
                    DescriptionPayload[] extra =
                        JsonConvert.DeserializeObject<DescriptionPayload[]>(desc.extra.ToString());
                    var build = "";
        ...
        private static byte ReadByte(ref int offset, IReadOnlyList<byte> buffer)
        {
            var b = buffer[offset];
            offset += 1;
            return b;
